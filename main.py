...
from database import (
    init_db, add_client, get_bookings_for, get_pending_reminders, mark_as_reminded,
    get_user_bookings, get_booking_for_cancel, cancel_user_booking
)
...
def ask_date(update: Update, context: CallbackContext):
    ...
    if not slots:
        update.message.reply_text("Kechirasiz, tanlangan kunda bo‘sh vaqtlar yo‘q.", reply_markup=ReplyKeyboardRemove())
        return ASK_DATE
...
def ask_time(update: Update, context: CallbackContext):
    ...
    msg = (
        ...
        f"👤 Ism: {udata['name']}\n"
        f"📞 Tel: {udata['phone']}\n"
        f"🛠 Xizmat: {udata['service']}\n"
        f"💈 Barber: {udata['barber']}\n"
        f"📅 Sana: {udata['date']}\n"
        f"⏰ Vaqt: {udata['time']}\n\n"
...
def finish(update: Update, context: CallbackContext):
    ...
    udata = context.user_data
    booked = get_bookings_for(udata['barber'], udata['date'])
    if udata['time'] in booked:
        update.message.reply_text("Afsuski, bu vaqt band. /start bilan boshqa vaqt tanlang.", reply_markup=ReplyKeyboardRemove())
        return ASK_DATE

    user_id = update.message.from_user.id
    add_client(udata['name'], udata['phone'], udata['service'], udata['barber'], udata['date'], udata['time'], telegram_id=user_id)
    ...
    update.message.reply_text("Rahmat! Sizning joyingiz band qilindi. 😊", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
def check_reminders(context: CallbackContext):
    ...
    for b in bookings:
        booking_datetime = TZ.localize(datetime.strptime(f"{b['date']} {b['time']}", "%Y-%m-%d %H:%M"))
        time_diff = booking_datetime - now

        if 0 < time_diff.total_seconds() <= 1800:
            mark_as_reminded(b["id"])  # 1 marta yuborish

            # Customer
            try:
                context.bot.send_message(
                    chat_id=b["telegram_id"],
                    text=f"📢 *Eslatma!*\nSiz bugun soat *{b['time']}* da sartaroshxonamizga yozilgansiz.",
                    parse_mode="Markdown"
                )
            except Exception:
...
# ==================
# MIJOZ BRONLARINI KO'RISH / CANCEL
# ==================
def my_bookings(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    bookings = get_user_bookings(user_id)
    if not bookings:
        update.message.reply_text("Sizda faol bronlar yo‘q.")
        return
    text = "📋 *Sizning bronlaringiz:*\n\n"
    for b in bookings:
        text += f"🆔 ID: `{b['id']}`\n📅 Sana: {b['date']}\n⏰ Vaqt: {b['time']}\n💈 Barber: {b['barber']}\n\n"
    text += "Bekor qilish uchun: /cancel ID"
    update.message.reply_text(text, parse_mode="Markdown")

def cancel_booking(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    if not context.args:
        update.message.reply_text("Bron ID ni kiriting. Masalan: /cancel 12")
        return
    booking_id = context.args[0]
    booking = get_booking_for_cancel(booking_id, user_id)
    if not booking:
        update.message.reply_text("Bron topilmadi yoki sizga tegishli emas.")
        return
    booking_datetime = TZ.localize(datetime.strptime(f"{booking['date']} {booking['time']}", "%Y-%m-%d %H:%M"))
    now = datetime.now(TZ)
    if (booking_datetime - now).total_seconds() < 3600:
        update.message.reply_text("⛔ 1 soatdan kam qolgan, bekor qilib bo‘lmaydi.")
        return
...
def main():
    ...
    dp.add_handler(CommandHandler("cancel", cancel_booking))
    dp.add_handler(conv)
    ...
    updater.bot.set_my_commands([
        ...
        BotCommand("cancel", "Bronni bekor qilish")
    ])
